...
## Somehow this one is more optimized, idk how. Edit: apparently the count function is implemented in native code
def checkDup(string):
    for x in string:
        if string.count(x) > 1:
            return True
...
## This one tries to use a dictionary. Edit: This one is terrible, best to stick with other ones
def checkDup2(string):
    dic = {}
    for x in range(len(string)):
        if string[x] in dic.keys():
            dic[string[x]] += 1
        else:
            dic[string[x]] = 1
...
## This one returns an index or -1, and this one is to be used with longestSub3
def checkDup3(string):
    count = 0
    for x in range(len(string)):
        count = 0
        for y in range(len(string)):
            if string[x] == string[y]:
                count += 1
            if count > 1:
                #print("The char is " + string[x] + " " + string[x+1])
                return x + 1
    return -1

################################# longestSub Functions############################################################
...
#Second Version (more optimized)
def longestSub2(s):
    length = len(s)
    count = 1
    for x in range(length+1, -1, -1):
        for y in range(count):
            if checkDup(s[y:x+y]) == False:
                return (len(s[y:x+y]))
        count += 1

#Third Version (even more optimized)
def longestSub3(s):
    length = len(s)
    largest = 0
    start = 0
    for x in range(length):
        index = checkDup3(s[start:x])
        if (index == -1):
            temp = len(s[start:x])
            if temp > largest:
                largest = temp
...
#Fourth Version (DOES NOT WORK, work in progress)
def longestSub4(s):
    length = len(s)
    mySet = set()
    myDict = dict()
    ...
    largest = 0
    for x in range(length):
        index = checkDup3(s[x:endIndex])
        if (index == -1):
            temp = len(s[x:endIndex])
            if temp > largest:
                largest = temp

            if (largest < max(myDict) ):
                #print(myDict)
                indexTup = (myDict[max(myDict)])[0]
                x = indexTup[0]
                endIndex = indexTup[1] 
                (myDict[max(myDict)]).pop(0)
                if (myDict[max(myDict)]) == []:
                    myDict.pop(max(myDict))
            else:
                #print(s[x:endIndex])
                return largest
        else:
            ...
            keyLength = index - startIndex + 1
            #mySet.add(keyLength)
            if keyLength in myDict.keys():
                myDict[keyLength].append((startIndex, index))
            else:
                myDict.update({keyLength:[(startIndex,index)]})          
...
#This one uses a dictionary and is the fastest one yet
def longestSubDict(s):
    largest = 0
    temp = 0
...
## This one might be just a litte slower than the last one
def longestSubDict2(s):
    largest = 0
    d = defaultdict(lambda: -1, {})
    for index,x in enumerate(s):
        if (d[x] == -1):
...
if __name__ == '__main__':
    ...
    s = "abcabcbbabcdefghii"
    #s = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~dfsdfrwrsjfghuyfi"
    
    # s = "Heabcdeflo" #should be 8
    # s = "abcabcbb" #should be 3
    # s = "bbtablud" #should be 6
    s = "aab"
    s= "dvdf"
    #s = "abcabcbb"
    s = "bbtablud"
